...
const stock_index = (req, res) => {
    Stock.find().sort({createdAt: -1}) // -1 bedeutet, dass die Reihfolge absteigend ist also von neu erstellte Stock zum Alten
    .then((result) => {
...
const stock_details = (req, res) => {
    ...
    Stock.findById(id)
        .then((result) => {
            res.render('stocks/details', {title:  `${result.title} Details`, stock: result})
        })
        .catch((err) => { console.log(err)})
}
...
const stock_addproduct_get = (req, res) => {
    ...
    Stock.findById(stockID)
        .then(result => res.render('stocks/addProduct', {title:"Add Product", stock: result}) )
        .catch(err => console.log(err))
}
